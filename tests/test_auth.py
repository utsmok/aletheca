...
class TestResolveAuth:
    """Test AlethecaClient._resolve_auth()."""

    def test_api_key_returns_query_parameter_auth(self):
        auth = AlethecaClient._resolve_auth("my-api-key")
    ...
    def test_none_returns_no_auth(self):
        auth = AlethecaClient._resolve_auth(None)
        assert isinstance(auth, NoAuth)

    def test_empty_string_returns_no_auth(self):
        auth = AlethecaClient._resolve_auth("")
        assert isinstance(auth, NoAuth)
...
class TestClientAuth:
    ...
    def test_client_with_api_key(self):
        with patch.object(AlethecaClient, "__init__", lambda self: None):
            AlethecaClient.__new__(AlethecaClient)
            auth = AlethecaClient._resolve_auth("test-key")
            assert isinstance(auth, QueryParameterAuth)
    ...
    def test_client_explicit_auth_strategy_takes_precedence(self):
        ...
        resolved = AlethecaClient._resolve_auth("some-key")
